...
export function registerTsTools(server: McpServer): void {
    ...
    server.registerTool(
        'hoist-get-members',
        {
            ...
            description: `List the properties, methods, and accessors of a class or interface with types, decorators, defaults, and JSDoc: own members, then members inherited via extends, then members from types outside hoist-react (React, Blueprint), each grouped by declaring type. Narrow with filter (substring of the member name, e.g. "col"), include: "own" | "inherited", and memberKind: "property" | "method" | "accessor". Listings of up to ${FULL_DETAIL_MAX_MEMBERS} members show full JSDoc; larger ones show one line per member unless detail: "full" is passed. React attribute groups (HTMLAttributes, DOMAttributes, AriaAttributes) are counted and listed only with a filter, so filter: "click" finds onClick on any Props interface. Members without JSDoc inherit it from an implemented interface or the sibling *Spec / *Config (Column.headerName from ColumnSpec). A const, function, type, or enum name returns an error naming the right tool. For signature, docs, import, and a member summary in one call use hoist-get-symbol.`,
            inputSchema: z.object({
                name: z
                    .string()
                    .describe(
