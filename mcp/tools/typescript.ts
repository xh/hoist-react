...
export function registerTsTools(server: McpServer): void {
    ...
    server.registerTool(
        'hoist-get-members',
        {
            ...
            description: `List the properties, methods, and accessors of a class or interface with types, decorators, defaults, and JSDoc: own members, then members inherited through extends grouped by declaring type, then members inherited from types outside hoist-react (React, Blueprint) found via the type checker and grouped by declaring type. Narrow with filter (case-insensitive substring on the member name, e.g. filter: "col"), include: "own" | "inherited", and memberKind: "property" | "method" | "accessor". Listings of up to ${FULL_DETAIL_MAX_MEMBERS} members show full JSDoc; larger ones (GridModel has ~180 members) show one line per member with the first sentence - narrow the filter, or pass detail: "full", for complete docs. Standard React attribute groups (HTMLAttributes, DOMAttributes, AriaAttributes) are counted but listed only when a filter is passed, so filter: "click" finds onClick on any Props interface. Class members without JSDoc inherit it from an implemented interface or the sibling *Spec / *Config interface (Column.headerName shows ColumnSpec.headerName docs). A const, function, type, or enum name returns an error naming the right tool. For the signature, top-level JSDoc, import line, and a member summary in one call, use hoist-get-symbol instead.`,
            inputSchema: z.object({
                name: z
                    .string()
                    .describe(
