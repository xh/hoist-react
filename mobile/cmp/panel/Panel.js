...
Panel.propTypes = {
     ...
     *     (current model must implement LoadSupport).
     */
...
function parseLoadDecorator(prop, name, contextModel) {
    ...
    if (prop === 'onLoad') {
        ...
        if (!loadModel) {
            console.warn(`Cannot use 'onLoad' for '${name}'.  Context model does not implement LoadSupport or does not have a 'loadModel' property.`);
            return null;
        }
