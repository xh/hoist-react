...
export interface DateInputProps extends HoistProps, HoistInputProps, StyleProps, LayoutProps {
    ...
    /**
     * Escape hatch to pass props directly to the underlying react-day-picker calendar - e.g.
     * `disabled` matchers (`{dayOfWeek: [0, 6]}` to disallow weekends), `captionLayout`, `locale`.
     * Merged over this component's own calendar defaults, except `disabled` matchers, which are
     * combined with those generated by minDate/maxDate. Selection wiring (`mode`, `selected`,
     * `onSelect`) is controlled by this component and cannot be overridden.
     */
