 ...
 *
 * The built-in aggregators are then run bottom-up over the structure's own children, against the
 * oracle, to prove property 2 is sufficient for them. Their standalone semantics are covered in
    ...
    for (const s of SCENARIOS) {
        ...
        for (let c = 0; c < structure.cellCount; c++) {
            if (structure.cellPath[c] === 0) {
                errs.push(
                    `cell ${c} materializes the root path, which must be the group row itself`
                );
                break;
            }
        }
...
//------------------
suite('exactly-once propagation', () => {
    for (const s of SCENARIOS) {
        const errs: string[] = [];
        ...
        for (let leaf = 0; leaf < spec.leafOwnerGroup.length && errs.length < 6; leaf++) {
            ...
            ancestors.forEach(g =>
                prefixes.forEach(p => {
                    ...
                    if (c == null) {
                        errs.push(
                            `leaf ${leaf}: no cell for group ${g} path "${paths.paths[p].key}"`
                        );
                    } else {
                        expectCells.add(c);
                    }
...
//------------------
suite('path ordering: ascending by value, empty segment last', () => {
    const errs: string[] = [],
        records = [
            rec('F1', 'S1', 'US', 'x'),
            rec('F1', 'S1', 'APAC', 'x'),
...
suite('path ordering: numeric values sort numerically, not lexically', () => {
    const errs: string[] = [],
...
suite('path keys are injective across every scenario', () => {
    const errs: string[] = [];
    for (const s of SCENARIOS) {
        const {paths} = discoverPivotPaths(s.records, s.pivotDims),
...
suite('cell field naming: root path binds to the bare value field', () => {
    const errs: string[] = [],
...
suite('emptyPathLabel is honored and keeps a distinct key', () => {
    const errs: string[] = [],
...
suite('maxPivotPaths: null disables the guard', () => {
    const errs: string[] = [];
    const records: Rec[] = [];
    for (let i = 0; i < 50; i++) records.push(rec('F1', 'S1', `R${i}`, 'x'));
    try {
...
suite('aggregation composes over the structure', () => {
    for (const s of SCENARIOS) {
        const errs: string[] = [],
            built = build(s),
        ...
        for (const [agg, field] of [
            ...
            for (let c = 0; c < structure.cellCount; c++) {
                const leaves = ref.get(
                        structure.cellGroup[c] * spec.pathCount + structure.cellPath[c]
                    ),
...
//------------------
suite('CHILD_COUNT on a cell counts only the children carrying that path', () => {
    const errs: string[] = [];
    let strictlyFewer = 0;
