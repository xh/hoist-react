...
- **Licensing:** Apache-2.0 (permissive, commercial-friendly). FLAG for EMC/compliance review: confirm the open-source engine (not the enterprise product) is what's adopted, and that Apache-2.0 + WASM binary distribution is acceptable in client deployment.
...
| **Glide Data Grid** (`@glideapps/glide-data-grid`) | MIT, canvas-based, fast - but it is a **renderer, not a data engine**, orthogonal to the data-layer question and a competitor to AG Grid, not to our Store/Cube. Likely NOT the EMC "fast WASM grid" (it is canvas-JS, not WASM). Mention as a rendering-tier datapoint only; do not spike for the data-layer question. |
...
**If the binding constraint is real-time streaming fanout (the §3.3 EMC pressure):**
