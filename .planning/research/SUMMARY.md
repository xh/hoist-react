...
The validated current architecture (per KICKOFF-VALIDATION.md) corrects several claims from the original brief that would have misdirected planning. Real-time delivery today is poll-then-diff over HTTP, not WebSocket push. Cube-to-View propagation is imperative push (`noteCubeUpdated`), not MobX observation; MobX observability enters only at the `View.result` boundary (`@observable.ref`). Weighted-average aggregation is a custom `Aggregator` subclass, not a built-in. These corrections sharpen the integration seam analysis: any new engine must feed MobX observability at the `View.result -> Store` boundary, and a genuine streaming push transport is itself a candidate change, not an existing capability to exploit.
...
**Rationale:** The validated architecture is the prerequisite for every measurement and comparison. Several claims from the original brief were inaccurate (WebSocket push, built-in weighted-avg, cube as MobX observable). Phase 1 resolves the two remaining explicit unknowns: the exact copy-vs-reuse map across the full data pipeline, and the MobX reaction granularity in the View.result -> Store -> GridModel -> AG Grid path.
...
- **True streaming push transport:** Today's poll-then-diff delivery (WebSocket is a notification channel only) means genuine real-time streaming is itself a candidate change. The Phase 5 strategy synthesis must assess whether a push transport is part of the Data 2.0 recommendation or a separate prerequisite.
