...
<task type="auto">
  <name>Task 1: Create doc resources and doc tools, update server, remove placeholders</name>
  <files>mcp/resources/docs.ts, mcp/tools/docs.ts, mcp/server.ts, mcp/resources/placeholder.ts, mcp/tools/placeholder.ts</files>
  <action>
...
**Also keep the hoist-ping tool** -- it's useful for connectivity testing. Move it into `mcp/tools/docs.ts` or keep `mcp/tools/placeholder.ts` for it. Simplest: include hoist-ping registration in `registerDocTools` alongside the doc tools, and delete `mcp/tools/placeholder.ts`.
...
Call `registerDocResources(server)` and `registerDocTools(server)` instead of the placeholder functions.

**Delete placeholder files:**
- Delete `mcp/resources/placeholder.ts`
- Delete `mcp/tools/placeholder.ts`

These are replaced by the doc-serving modules. The hoist-ping tool should be preserved (moved into docs.ts or a separate utility tools file).
  </action>
  <verify>
...
3. Verify placeholder files are deleted.
4. Verify `mcp/server.ts` imports from docs modules, not placeholders.
  </verify>
  <done>
- `mcp/resources/docs.ts` registers doc-index, conventions, and hoist-doc template resources
- `mcp/tools/docs.ts` registers hoist-search-docs and hoist-list-docs tools (plus hoist-ping)
- `mcp/server.ts` imports and calls the new registration functions
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 2: Verify MCP documentation serving with Claude Code</name>
  <files>none</files>
  <action>
