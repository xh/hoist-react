...
last_updated: "2026-07-02T15:53:27.357Z"
progress:
  total_phases: 8
  completed_phases: 2
  total_plans: 19
...
**Current focus:** Phase 03 — baseline-performance-envelope

## Current Position

Phase: 03 (baseline-performance-envelope) — EXECUTING
Plan: 1 of 7
...
- [Phase 02-measurement-harness, post-close polish]: Corrected the Data Lab demo persistence to fit the
  right primitives. Run history had been persisted through a ViewManager (one synthesized-name view per
  run, plus a parallel `savedRuns` mirror that only re-hydrated the single active view) - a category
  error, since runs are transient, un-named measurement records, not named/shared/curated views. Dropped
  `runViewManager` (from `AppModel` + `DataLabModel`); `savedRuns` now persists to localStorage via
  `@persist.with({localStorageKey: 'dataLab.savedRuns'})` (auto-hydrates; auto-clears the entry when
  emptied), with a confirm-guarded Clear History control. Also removed the redundant custom "Save Profile"
  button/helpers in favor of the built-in `viewManager()` controls already mounted in the app bar -
  scenario profiles are now purely ViewManager-driven. Principle: profiles = named/shared/curated state
