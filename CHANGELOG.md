...
* Added support for Correlation IDs across fetch requests and error / activity tracking:
    ...
    * New `FetchService.autoGenerateCorrelationIds` and `correlationIdHeaderKey` properties to
    ...
    * Correlation ID assignment is available via:
        * `FetchOptions.correlationId` - specify an ID to be used on a particular request or `true`
          to use a UUID generated by Hoist (see `XH.genUUID()`).
        * `LoadSpec.correlationId` - provide an ID or `true` to core data-loading APIs such as
          `loadAsync()`. The resulting `LoadSpec` can then be passed to multiple `FetchService`
          calls to apply a single ID to multiple, related requests.
