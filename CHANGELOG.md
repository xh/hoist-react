...
* Added support for Correlation IDs across fetch requests and error / activity tracking:
    ...
    * New `FetchService.autoGenerateCorrelationIds()` and `setCorrelationIdHeaderKey()` APIs
      support auto-generation and inclusion of Correlation IDs on outbound request headers.
    * More selective control over correlation IDs is also available via:
