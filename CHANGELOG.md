...
* Added support for Correlation IDs across fetch requests and error / activity tracking:
    ...
    * Correlation IDs are assigned via:
        ...
        * `FetchOptions.trackOptions` - specify a `TrackOptions` object to track a request via
          Hoist activity tracking. The request's Correlation ID will be included automatically.
        * `TrackOptions.correlationId` - specify an ID for a tracked activity, if not using the
          above-mentioned `FetchOptions.trackOptions` API.
