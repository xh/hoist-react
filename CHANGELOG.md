...
:octocat: [Commit Log](https://github.com/exhi/hoist-react/compare/v12.0.0...v12.1.0)
...
* User customizations to column widths are now saved if the GridModel has been configured with a `stateModel` key or model instance - see [`GridStateModel`](https://github.com/exhi/hoist-react/blob/master/desktop/cmp/grid/GridStateModel.js).
...
* `HoistComponent` now provides a `getClassNames()` method that will merge any `baseCls` CSS class names specified on the component with any instance-specific classes passed in via props (#252). 
  * Components that wish to declare and support a `baseCls` should use this method to generate and apply a combined list of classes to their outermost rendered elements (e.g. [this usage](https://github.com/exhi/hoist-react/blob/e8de195367d869e6eaef17267a9bd67068c03226/desktop/cmp/grid/Grid.js#L83) within `Grid`).
...
:octocat: [Commit Log](https://github.com/exhi/hoist-react/compare/v10.0.1...v11.0.0)
...
* **The Context menu API has changed.** The [`ContextMenuSupport` mixin](https://github.com/exhi/hoist-react/blob/develop/desktop/cmp/contextmenu/ContextMenuSupport.js) now specifies an abstract `getContextMenuItems()` method for component implementation (replacing the previous `renderContextMenu()` method). See the new [`ContextMenuItem` class](https://github.com/exhi/hoist-react/blob/develop/desktop/cmp/contextmenu/ContextMenuItem.js) for what these items support, as well as several static default items that can be used.
...
:octocat: [Commit Log](https://github.com/exhi/hoist-react/compare/v8.0.0...v9.0.0)
...
:octocat: [Commit Log](https://github.com/exhi/hoist-react/compare/v7.2.0...v8.0.0)
...
:octocat: [Commit Log](https://github.com/exhi/hoist-react/compare/v7.1.0...v7.2.0)
...
* **Added [`AppBar` component](https://github.com/exhi/hoist-react/blob/develop/cmp/appbar/AppBar.js)** to help further standardize a pattern for top-level application headers.
...
* **We updated the APIs for core MobX helper methods added to component/model/service classes.** In particular, `addReaction()` was updated to take a more declarative / clear config object. 8169123a4a8be6940b747e816cba40bd10fa164e
    * See [Reactive.js](https://github.com/exhi/hoist-react/blob/develop/core/mixins/Reactive.js) - the mixin that provides this functionality.
