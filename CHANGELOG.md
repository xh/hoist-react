...
* `FilterChooserModel` has been enhanced to better handle multiple simultaneous filters with
  different `op`s on the same field.  "Inclusive" ops (e.g. `=`, `like` ) will be OR'ed together and
  "Exclusive" ops (e.g. `!=`, 'not like`) will be AND'ed together.  This behavior is consistent with
  current behavior and user intuition, and should maximize the ability to create useful queries.
