 ...
 *
 * @param {*} [modelClass] - class of model to be returned.  If not provided the 'closest' inherited
 *      model will be returned.
 * @param {Array} [props] - provide to do initial search in props.
 *
 * @returns model or null if no matching model found.
 */
export function useModel(modelClass, props) {
    const modelLookup = useContext(ModelLookupContext),
        [ret] = useState(() => {
            ...
            if (modelLookup) {
                return modelLookup.lookupModel(modelClass);
            }
            return null;
        });
