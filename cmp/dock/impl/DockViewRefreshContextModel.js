 ...
 */
@RefreshContextModel
export class DockViewRefreshContextModel {

    dockViewModel;

    constructor(dockViewModel)  {
        this.dockViewModel = dockViewModel;
        this.addReaction({
            track: () => dockViewModel.isActive,
            run: this.noteActiveChanged
        });
    }

    async doLoadAsync(loadSpec) {
        const {dockViewModel} = this,
            mode = dockViewModel.refreshMode;

        if (dockViewModel.isActive || mode === RefreshMode.ALWAYS) {
    ...
    noteActiveChanged(isActive) {
        if (isActive) {
            const mode = this.dockViewModel.refreshMode;
